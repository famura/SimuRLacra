...
def plot_2d_thetas(proposals, obs_thetas=None, marginal_samples=None):
    ...
    for obs in proposals:
        ax.scatter(obs[:, 0], obs[:, 1], c=colors[cnt], label="Samples p(theta|x_o_{}".format(cnt + 1), alpha=0.3)
        cnt += 1

    # plot marginal samples
    if marginal_samples is not None:
        ax.scatter(marginal_samples[:, 0], marginal_samples[:, 1], c=colors[-1], label="Marg. Samples", alpha=0.3)
    ...
    if obs_thetas is not None:
        ...
        for obs in obs_thetas:
            ax.scatter(obs[0], obs[1], facecolors=colors[cnt], marker="X", edgecolors="black", s=200)
            cnt += 1
