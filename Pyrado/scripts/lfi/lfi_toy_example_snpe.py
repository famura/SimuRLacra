...
class ToySimulator(Simulator):
    ...
    def __init__(self):
        super().__init__(strategy="states")
    ...
    def __call__(self, params) -> Union[StepSequence, to.Tensor]:
        with to.no_grad():
            ...
            s1 = params[2] ** 2
            s2 = params[3] ** 2
            rho = to.tanh(params[4])
            cov12 = rho * s1 * s2
            covariance_matrix = to.tensor([[s1 ** 2, cov12], [cov12, s2 ** 2]])
            dist = MultivariateNormal(loc=mean, covariance_matrix=covariance_matrix)
            x = dist.sample((4,)).view(-1, 1).squeeze().numpy()
            ro = StepSequence(rewards=to.empty(x.shape),
                              actions=to.empty(x.shape),
                              observations=x)
...
if __name__ == "__main__":
    ...
    if not args.eval:
        ...
        inference.step(snapshot_mode="latest",
                       meta_info=dict(rollouts_real=ro_real),
                       logging=True,
                       data_representation="states")
    ...
    # sample parameters
    sample_params, _, sim_rollouts = inference.evaluate(meta_info=dict(rollouts_real=ro_real),
                                                        num_samples=num_samples,
                                                        compute_quantity={"sample_params": True, "sim_traj": True})
